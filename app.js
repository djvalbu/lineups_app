...
let state=null;
let ui={view:'dashboard',matchId:null,reportIndex:0};
...
function normalizeState(){
  ...
  state.matches.forEach(m=>{m.positions=m.positions||[];m.subGoals=m.subGoals||[];m.positions.forEach(p=>{p.events=p.events||[]})});
...
function addMatch(){const m={id:uid('match'),analysisId:state.analysis?.id,kind:'MATCH',roundNumber:'',opponentName:'',opponentNameEn:'',goalsFor:0,goalsAgainst:0,result:'D',formation:'1-4-4-2',sortOrder:Math.max(0,...state.matches.filter(x=>x.kind==='MATCH').map(x=>x.sortOrder||0))+1,positions:[],subGoals:[]};state.matches.push(m);ui.matchId=m.id;saveState();renderAll();showView('matches');toast('Match added')}
...
function eventStacks(pos,kind='mini'){
 ...
 const goals=`<div class="goal-stack ${isGk?'gk':''}">${gf.length?`<span class="goal-line for"><img src="${EVENT_META.GOAL_FOR.icon}">${minuteString(gf)}</span>`:''}${ga.length?`<span class="goal-line against"><img src="${EVENT_META.GOAL_AGAINST.icon}">${minuteString(ga)}</span>`:''}</div>`;
 return `${left?`<div class="event-stack ${isGk?'gk':''}">${left}</div>`:''}${gf.length||ga.length?goals:''}`
}
...
function reportPageHtml(m){const tpl=matchTemplate(m);let title='';if(m.kind==='EXPECTED')title=state.settings?.reportTitleExpected||'预计首发名单';else if(m.kind==='SUBS')title=state.settings?.reportTitleSubs||'主要替补球员';else title=`${m.roundNumber||''}_ VS ${m.opponentName||''} _ ${m.goalsFor||0}-${m.goalsAgainst||0} (${RESULT_CN[m.result]||'平'})`;
 const players=(m.positions||[]).map(pos=>{const slot=tpl[pos.positionIndex];if(!slot)return'';return reportPlayerHtml(pos,slot)}).join('');return `<div class="report-page"><img class="field-img" src="assets/field_exact_4x.jpg"><div class="report-title">${escapeHtml(title)}</div>${players}</div>`}
...
async function addPptxPlayer(pptx,slide,p,pos,slot){const g=reportPlayerGeom(slot),cx=g.left,cy=g.top+76.119;const ST=pptx.ShapeType;if(p.photoAsset){const img=await urlToDataUri(p.photoAsset);slide.addImage({data:img,x:pt(g.left+22.438),y:pt(g.top),w:pt(75),h:pt(75),transparency:0})}
 ...
 const gf=eventsFor(pos,EVENT_TYPES.GOAL_FOR),ga=eventsFor(pos,EVENT_TYPES.GOAL_AGAINST);const isGk=slot.label==='GK';let gy=g.top+(isGk?52:6);const gx=g.left+(isGk?104:105),tx=g.left+(isGk?121:122);if(gf.length){const data=await urlToDataUri(EVENT_META.GOAL_FOR.icon);slide.addImage({data,x:pt(gx),y:pt(gy),w:pt(15),h:pt(15)});slide.addText(minuteString(gf),{x:pt(tx),y:pt(gy-1),w:pt(80),h:pt(17),fontFace:'Arial',fontSize:9,bold:true,color:'0433FF',margin:0,fill:transparent,line:{color:'FFFFFF',transparency:100},breakLine:false,fit:'shrink'});gy+=18}if(ga.length){const data=await urlToDataUri(EVENT_META.GOAL_AGAINST.icon);slide.addImage({data,x:pt(gx),y:pt(gy),w:pt(15),h:pt(15)});slide.addText(minuteString(ga),{x:pt(tx),y:pt(gy-1),w:pt(80),h:pt(17),fontFace:'Arial',fontSize:9,bold:true,color:'FF1900',margin:0,fill:transparent,line:{color:'FFFFFF',transparency:100},breakLine:false,fit:'shrink'})}}
